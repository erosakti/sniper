--[[ 
    ...
    Fitur: Horizontal UI, Anti-Stuck, Safe GUI (No Plugin Error), Custom Pet Price
]]
...
-- ==================================================================
local ITEM_LIST = {
    "Giant Scorpion","Rainbow Dilophosaurus","Rainbow Elephant","Ghostly Headless Horseman","Rainbow Birb","Seal","Flamingo","Toucan","Sea Turtle","Orang Utan","Mimic Octopus","Kitsune","Raccoon","Peryton","Gilded Choc Peryton","Disco Bee"
}
...
local function StartSealSniperV120()
    ...
    -- 🔥 AUTO-MIGRASI DATA: Ubah struktur target lama ke format Dictionary untuk Custom Price
    local cleanTargets = {}
    for k, v in pairs(getgenv().SniperConfig.Targets) do
        if type(k) == "number" and type(v) == "string" then
            cleanTargets[v] = 0 -- 0 berarti ikuti Global Max Price
    ...
    local function ToggleTarget(name, button, priceBox)
        if getgenv().SniperConfig.Targets[name] ~= nil then
            getgenv().SniperConfig.Targets[name] = nil
            button.BackgroundColor3 = Color3.fromRGB(40, 40, 45); button.TextColor3 = Color3.fromRGB(200, 200, 200)
            priceBox.Visible = false
        else
            getgenv().SniperConfig.Targets[name] = tonumber(priceBox.Text) or 0
            button.BackgroundColor3 = Color3.fromRGB(0, 180, 100); button.TextColor3 = Color3.fromRGB(255, 255, 255)
            priceBox.Visible = true
        end
        SaveConfig()
    end

    -- 🔥 [NEW] RENDER UI CUSTOM PRICE 🔥
    for _, item in pairs(ITEM_LIST) do
        local itemContainer = Instance.new("Frame"); itemContainer.Parent = ScrollFrame; itemContainer.Size = UDim2.new(1, -6, 0, 20); itemContainer.BackgroundTransparency = 1
        
        local btn = Instance.new("TextButton"); btn.Parent = itemContainer; btn.Size = UDim2.new(1, -45, 1, 0); btn.Font = Enum.Font.Gotham; btn.Text = item; btn.TextSize = 10; btn.TextTruncate = Enum.TextTruncate.AtEnd
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
        
        local priceBox = Instance.new("TextBox"); priceBox.Parent = itemContainer; priceBox.Size = UDim2.new(0, 40, 1, 0); priceBox.Position = UDim2.new(1, -40, 0, 0); priceBox.Font = Enum.Font.GothamBold; priceBox.TextSize = 9; priceBox.PlaceholderText = "Max"
        priceBox.BackgroundColor3 = Color3.fromRGB(20, 20, 25); priceBox.TextColor3 = Color3.fromRGB(255, 255, 0)
        Instance.new("UICorner", priceBox).CornerRadius = UDim.new(0, 4)
        
        if IsSelected(item) then 
            btn.BackgroundColor3 = Color3.fromRGB(0, 180, 100); btn.TextColor3 = Color3.fromRGB(255, 255, 255)
            priceBox.Visible = true
            local savedPrice = getgenv().SniperConfig.Targets[item]
            if savedPrice and savedPrice > 0 then priceBox.Text = tostring(savedPrice) else priceBox.Text = "" end
        else 
            btn.BackgroundColor3 = Color3.fromRGB(40, 40, 45); btn.TextColor3 = Color3.fromRGB(200, 200, 200) 
            priceBox.Visible = false
        end
        
        btn.MouseButton1Click:Connect(function() ToggleTarget(item, btn, priceBox) end)
        
        priceBox.FocusLost:Connect(function()
            if IsSelected(item) then
                getgenv().SniperConfig.Targets[item] = tonumber(priceBox.Text) or 0
                SaveConfig()
            end
        end)
    end
    ...
    local function processBoothData(player, data)
        ...
        for listingUUID, info in pairs(data.Listings) do
            ...
            if linkID and data.Items[linkID] then
                ...
                -- 🔥 [NEW] LOGIC CEK TARGET & CUSTOM PRICE 🔥
