--[[ 
    🛡️ SEAL SNIPER V135 (AUTO-TELEPORT TO GARDEN)
    Base: Source Code V120 Original
    Modifikasi: 
    - Auto-Extract Pet Database via RAM
    - Fitur Search Box pada menu Dropdown Pet
    - SMART HOP: Auto-Find Seller berdasarkan list Active Targets
    - ROLLING TARGETS: Mengacak urutan target tiap hop
    - DISCORD PING: Kolom input User ID untuk notifikasi tag otomatis
    - CLEAR TARGETS: Tombol pintas untuk menghapus semua pet
    - EGG PRESETS: Tambahkan seluruh pet sekaligus berdasarkan tipe Telur
    - ADVANCED WEBHOOK: Menampilkan Username Pembeli & Sisa Token
    - GUI SCRAPING TOKEN: Membaca Token langsung dari UI layar
    - AUTO-HOP ON STUCK: Jika 100x gagal beli, langsung pindah server
    - TIMESTAMP: Menambahkan waktu realtime pada notifikasi Discord
    - CUSTOM HOP LIMIT: Bisa mengatur batas maksimal server hop
    - [NEW] AUTO-TELEPORT GARDEN: Otomatis pulang ke map kebun saat batas Hop tercapai
]]
...
-- ==================================================================
-- 🔍 SISTEM EKSTRAKSI DATABASE (PETS + EGGS)
-- ==================================================================
local function FetchMasterPetData()
    local rawList = {}
    local rawEggs = {}
            ...
            pcall(function()
                ...
                for key, val in pairs(t) do
                    if type(key) == "string" and type(val) == "table" then
                        local eType = rawget(val, "EggType")
                        if eType or rawget(val, "HatchTime") then
                            table.insert(tempNames, {name = key, egg = eType})
                            isPetDatabase = true
                        end
                    end
                end
                ...
                if isPetDatabase and #tempNames > 5 then
                    for _, data in ipairs(tempNames) do 
                        table.insert(rawList, data.name)
                        if data.egg and type(data.egg) == "string" then
                            rawEggs[data.egg] = rawEggs[data.egg] or {}
                            rawEggs[data.egg][data.name] = true
                        end
                    end
                end
            end)
    ...
    if #rawList == 0 then
        ...
        rawEggs["Rare Summer"] = {["Sea Turtle"]=true, ["Seal"]=true, ["Toucan"]=true, ["Orang Utan"]=true, ["Flamingo"]=true}
    end
    ...
    local finalEggs = {}
    for eggName, pets in pairs(rawEggs) do
        local petList = {}
        for p, _ in pairs(pets) do table.insert(petList, p) end
        finalEggs[eggName] = petList
        table.insert(finalSorted, 1, "[EGG] " .. eggName)
    end
    
    return finalSorted, finalEggs
end
...
local function StartSealSniperV120()
    ...
    -- 👇 GANTI ANGKA NOL DI BAWAH INI DENGAN PLACE ID MAP KEBUN KAMU 👇
    local GARDEN_PLACE_ID = 126884695634066 
    -- 👆==========================================================👆

    local DefaultConfig = {
        Running = false, AutoHop = true, Targets = {}, MaxPrice = 1,
        Delay = 0.0, HopDelay = 15, WebhookUrl = "", DiscordID = "",
        MaxHops = 0, CurrentHops = 0
    }
    ...
    local function SendWebhook(itemName, price, seller)
        ...
        local pingID = getgenv().SniperConfig.DiscordID
        local contentMsg = ""
        if pingID and pingID ~= "" then contentMsg = "<@" .. pingID .. "> 🎉 Bot berhasil membeli pet!" end

        local buyerName = LocalPlayer.Name
        local tokenBalance = "Unknown"
        ...
        pcall(function()
            ...
            if pg then
                local uiMain = pg:FindFirstChild("TradeTokenCurrency_UI")
                if uiMain and uiMain:FindFirstChild("TradeTokens") and uiMain.TradeTokens:FindFirstChild("TextLabel1") then
                    tokenBalance = uiMain.TradeTokens.TextLabel1.Text
                    return
                end
                
                local uiBuy = pg:FindFirstChild("BuyTokens")
                if uiBuy and uiBuy:FindFirstChild("Frame") and uiBuy.Frame:FindFirstChild("Label") then
                    local txt = uiBuy.Frame.Label.Text
                    tokenBalance = string.gsub(txt, "Tokens: ", "")
                    return
                end
                
                local uiBooth = pg:FindFirstChild("TradeBooth")
                if uiBooth and uiBooth:FindFirstChild("Frame") and uiBooth.Frame:FindFirstChild("TokenCounter") and uiBooth.Frame.TokenCounter:FindFirstChild("Label") then
                    local txt = uiBooth.Frame.TokenCounter.Label.Text
                    tokenBalance = string.gsub(txt, "Tokens: ", "")
                    return
                end
            end
        ...
        local data = {
            ["content"] = contentMsg,
            ["embeds"] = {{
                ["title"] = "🛡️ SNIPE SUCCESS!", 
                ["description"] = "✅ Berhasil mendapatkan **" .. itemName .. "**", 
                ["color"] = 65280, 
                ["fields"] = {
                    {["name"] = "💰 Price", ["value"] = tostring(price), ["inline"] = true}, 
                    {["name"] = "👤 Seller", ["value"] = seller, ["inline"] = true},
                    {["name"] = "🛒 Buyer", ["value"] = buyerName, ["inline"] = true},
                    {["name"] = "🪙 Current Tokens", ["value"] = tokenBalance, ["inline"] = true}
                }, 
                ["footer"] = {["text"] = "Seal Sniper V135"},
    ...
    local Title = Instance.new("TextLabel"); Title.Parent = MainFrame; Title.BackgroundTransparency = 1; Title.Position = UDim2.new(0, 10, 0, 5); Title.Size = UDim2.new(0, 150, 0, 20); Title.Font = Enum.Font.GothamBold; Title.Text = "BOT V135 🛡️"; Title.TextColor3 = Color3.fromRGB(100, 255, 100); Title.TextSize = 13; Title.TextXAlignment = Enum.TextXAlignment.Left
    ...
    -- 🔥 [UI DROPDOWN & SEARCH] 🔥
    local DynamicPetList, EggPresets = FetchMasterPetData()
    local SelectedPet = "Mimic Octopus"
    if table.find(DynamicPetList, "Mimic Octopus") then SelectedPet = "Mimic Octopus" elseif #DynamicPetList > 0 then SelectedPet = DynamicPetList[1] end
    
    local function IsSelected(name) return getgenv().SniperConfig.Targets[name] ~= nil end

    local DropdownBtn = Instance.new("TextButton"); DropdownBtn.Parent = MainFrame; DropdownBtn.Position = UDim2.new(0, 10, 0, 35); DropdownBtn.Size = UDim2.new(0, 160, 0, 25); DropdownBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 45); DropdownBtn.Font = Enum.Font.GothamSemibold; DropdownBtn.TextSize = 10; DropdownBtn.TextColor3 = Color3.fromRGB(220, 220, 220); DropdownBtn.TextXAlignment = Enum.TextXAlignment.Left; DropdownBtn.ClipsDescendants = true; Instance.new("UICorner", DropdownBtn).CornerRadius = UDim.new(0, 4)
    ...
    local ActiveTargetsLbl = Instance.new("TextLabel"); ActiveTargetsLbl.Parent = MainFrame; ActiveTargetsLbl.Position = UDim2.new(0, 10, 0, 95); ActiveTargetsLbl.Size = UDim2.new(0, 160, 0, 95); ActiveTargetsLbl.BackgroundTransparency = 1; ActiveTargetsLbl.Font = Enum.Font.Gotham; ActiveTargetsLbl.TextSize = 9; ActiveTargetsLbl.TextColor3 = Color3.fromRGB(150, 150, 150); ActiveTargetsLbl.TextXAlignment = Enum.TextXAlignment.Left; ActiveTargetsLbl.TextYAlignment = Enum.TextYAlignment.Top; ActiveTargetsLbl.TextWrapped = true; ActiveTargetsLbl.ZIndex = 1
    
    local ClearTargetsBtn = Instance.new("TextButton"); ClearTargetsBtn.Parent = MainFrame; ClearTargetsBtn.Position = UDim2.new(0, 10, 0, 195); ClearTargetsBtn.Size = UDim2.new(0, 160, 0, 20); ClearTargetsBtn.BackgroundColor3 = Color3.fromRGB(130, 40, 40); ClearTargetsBtn.Font = Enum.Font.GothamBold; ClearTargetsBtn.Text = "🗑️ CLEAR ALL TARGETS"; ClearTargetsBtn.TextColor3 = Color3.fromRGB(255, 255, 255); ClearTargetsBtn.TextSize = 9; ClearTargetsBtn.ZIndex = 1; Instance.new("UICorner", ClearTargetsBtn).CornerRadius = UDim.new(0, 4)

    local SearchBox = Instance.new("TextBox"); SearchBox.Parent = MainFrame; SearchBox.Position = UDim2.new(0, 10, 0, 62); SearchBox.Size = UDim2.new(0, 160, 0, 25); SearchBox.BackgroundColor3 = Color3.fromRGB(30, 30, 35); SearchBox.TextColor3 = Color3.fromRGB(255, 255, 255); SearchBox.Font = Enum.Font.Gotham; SearchBox.TextSize = 10; SearchBox.PlaceholderText = "🔍 Cari Pet / Egg..."; SearchBox.Visible = false; SearchBox.ZIndex = 11; Instance.new("UICorner", SearchBox).CornerRadius = UDim.new(0, 4)
    ...
    local function RefreshDropdownUI()
        ...
        if string.sub(SelectedPet, 1, 6) == "[EGG] " then
            PetToggleBtn.Text = "➕ ADD EGG"
            PetToggleBtn.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
        ...
        for _, child in pairs(DropListScroll:GetChildren()) do
            if child:IsA("TextButton") then
                if string.sub(child.Name, 1, 6) == "[EGG] " then
                    child.TextColor3 = Color3.fromRGB(0, 200, 255)
                elseif IsSelected(child.Name) then 
                    child.TextColor3 = Color3.fromRGB(0, 255, 100) 
                else 
    ...
    local function PopulateDropdown(filterText)
        ...
        for _, child in pairs(DropListScroll:GetChildren()) do if child:IsA("TextButton") then child:Destroy() end end
        ...
        for _, item in ipairs(DynamicPetList) do
            if filterText == "" or string.find(string.lower(item), filterText, 1, true) then
                ...
                if string.sub(item, 1, 6) == "[EGG] " then
                    btn.TextColor3 = Color3.fromRGB(0, 200, 255)
                elseif IsSelected(item) then 
                    btn.TextColor3 = Color3.fromRGB(0, 255, 100) 
                else 
                    btn.TextColor3 = Color3.fromRGB(220, 220, 220) 
                end
                
                btn.MouseButton1Click:Connect(function() SelectedPet = item; DropListScroll.Visible = false; SearchBox.Visible = false; SearchBox.Text = ""; RefreshDropdownUI() end)
            end
        end
        DropListScroll.CanvasSize = UDim2.new(0, 0, 0, UIListLayout.AbsoluteContentSize.Y + 10)
    end

    SearchBox:GetPropertyChangedSignal("Text"):Connect(function() PopulateDropdown(SearchBox.Text) end)

    DropdownBtn.MouseButton1Click:Connect(function() 
        local isOpening = not DropListScroll.Visible; DropListScroll.Visible = isOpening; SearchBox.Visible = isOpening
        if isOpening then PopulateDropdown(SearchBox.Text) end
    end)

    PetToggleBtn.MouseButton1Click:Connect(function()
        if string.sub(SelectedPet, 1, 6) == "[EGG] " then
            local eggName = string.sub(SelectedPet, 7)
            local petsInEgg = EggPresets[eggName]
            
            if petsInEgg then
                local priceToSet = tonumber(PetPriceBox.Text) or 0
                for _, p in ipairs(petsInEgg) do
                    getgenv().SniperConfig.Targets[p] = priceToSet
                end
                SaveConfig()
                
                PetToggleBtn.Text = "✅ ADDED!"
                PetToggleBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
                task.delay(1, function() if SelectedPet == "[EGG] " .. eggName then RefreshDropdownUI() end end)
            end
        else
    ...
    PetPriceBox.FocusLost:Connect(function()
        if string.sub(SelectedPet, 1, 6) == "[EGG] " then
            -- Biarkan user klik tombol ADD EGG
        elseif IsSelected(SelectedPet) then 
    ...
    ClearTargetsBtn.MouseButton1Click:Connect(function()
        getgenv().SniperConfig.Targets = {} 
        SaveConfig() 
        RefreshDropdownUI() 
        ClearTargetsBtn.Text = "✅ CLEARED!"
        ClearTargetsBtn.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
        task.delay(1, function() 
            ClearTargetsBtn.Text = "🗑️ CLEAR ALL TARGETS" 
            ClearTargetsBtn.BackgroundColor3 = Color3.fromRGB(130, 40, 40)
        end)
    end)
    ...
    local InputMaxHops = Instance.new("TextBox"); InputMaxHops.Parent = MainFrame; InputMaxHops.Position = UDim2.new(0, X_OFFSET, 0, 95); InputMaxHops.Size = UDim2.new(0, 190, 0, 25); InputMaxHops.Font = Enum.Font.GothamBold; InputMaxHops.TextSize = 10; InputMaxHops.Text = tostring(getgenv().SniperConfig.MaxHops or 0); InputMaxHops.PlaceholderText = "Max Hops (0 = Infinite)"; InputMaxHops.TextColor3 = Color3.fromRGB(255, 150, 50); InputMaxHops.BackgroundColor3 = Color3.fromRGB(30, 30, 35); Instance.new("UICorner", InputMaxHops).CornerRadius = UDim.new(0,4)
    InputMaxHops.FocusLost:Connect(function() getgenv().SniperConfig.MaxHops = tonumber(InputMaxHops.Text) or 0; SaveConfig() end)

    local InputWebhook = Instance.new("TextBox"); InputWebhook.Parent = MainFrame; InputWebhook.Position = UDim2.new(0, X_OFFSET, 0, 125); InputWebhook.Size = UDim2.new(0, 105, 0, 25); InputWebhook.Font = Enum.Font.GothamBold; InputWebhook.TextSize = 9; InputWebhook.Text = getgenv().SniperConfig.WebhookUrl or ""; InputWebhook.PlaceholderText = "Webhook URL"; InputWebhook.TextColor3 = Color3.fromRGB(200, 100, 255); InputWebhook.BackgroundColor3 = Color3.fromRGB(30, 30, 35); InputWebhook.ClipsDescendants = true; InputWebhook.ClearTextOnFocus = false; Instance.new("UICorner", InputWebhook).CornerRadius = UDim.new(0,4)
    InputWebhook.FocusLost:Connect(function() getgenv().SniperConfig.WebhookUrl = InputWebhook.Text; SaveConfig() end)

    local InputPing = Instance.new("TextBox"); InputPing.Parent = MainFrame; InputPing.Position = UDim2.new(0, X_OFFSET + 110, 0, 125); InputPing.Size = UDim2.new(0, 80, 0, 25); InputPing.Font = Enum.Font.GothamBold; InputPing.TextSize = 9; InputPing.Text = getgenv().SniperConfig.DiscordID or ""; InputPing.PlaceholderText = "UserID Ping"; InputPing.TextColor3 = Color3.fromRGB(255, 255, 255); InputPing.BackgroundColor3 = Color3.fromRGB(45, 45, 50); InputPing.ClearTextOnFocus = false; Instance.new("UICorner", InputPing).CornerRadius = UDim.new(0,4)
    InputPing.FocusLost:Connect(function() getgenv().SniperConfig.DiscordID = InputPing.Text; SaveConfig() end)

    local HopBtn = Instance.new("TextButton"); HopBtn.Parent = MainFrame; HopBtn.Position = UDim2.new(0, X_OFFSET, 0, 155); HopBtn.Size = UDim2.new(0, 90, 0, 25); HopBtn.Font = Enum.Font.GothamBold; HopBtn.TextSize = 9; Instance.new("UICorner", HopBtn).CornerRadius = UDim.new(0,4)
    local FPSBtn = Instance.new("TextButton"); FPSBtn.Parent = MainFrame; FPSBtn.Position = UDim2.new(0, X_OFFSET + 100, 0, 155); FPSBtn.Size = UDim2.new(0, 90, 0, 25); FPSBtn.Font = Enum.Font.GothamBold; FPSBtn.TextSize = 9; Instance.new("UICorner", FPSBtn).CornerRadius = UDim.new(0,4)
    local ToggleBtn = Instance.new("TextButton"); ToggleBtn.Parent = MainFrame; ToggleBtn.Position = UDim2.new(0, X_OFFSET, 0, 185); ToggleBtn.Size = UDim2.new(0, 190, 0, 30); ToggleBtn.Font = Enum.Font.GothamBlack; ToggleBtn.TextSize = 14; Instance.new("UICorner", ToggleBtn).CornerRadius = UDim.new(0,4)
    
    local StatusLbl = Instance.new("TextLabel"); StatusLbl.Parent = MainFrame; StatusLbl.BackgroundTransparency = 1; StatusLbl.Position = UDim2.new(0, X_OFFSET, 0, 220); StatusLbl.Size = UDim2.new(0, 190, 0, 15); StatusLbl.Font = Enum.Font.Gotham; StatusLbl.Text = "IDLE"; StatusLbl.TextColor3 = Color3.fromRGB(150, 150, 150); StatusLbl.TextSize = 10; StatusLbl.TextWrapped = true; StatusLbl.TextYAlignment = Enum.TextYAlignment.Top
    ...
    ToggleBtn.MouseButton1Click:Connect(function() 
        ...
        if getgenv().SniperConfig.Running then
            getgenv().SniperConfig.CurrentHops = 0 
        end
        SaveConfig()
        UpdateUI() 
    end)
    ...
    -- ==========================================
    -- 🚀 SMART SERVER HOP + ROLLING TARGETS
    -- ==========================================
    ...
    local function ServerHop()
        -- [UPDATE V135] AUTO-TELEPORT KE KEBUN JIKA MENCAPAI LIMIT HOP
        if getgenv().SniperConfig.MaxHops and getgenv().SniperConfig.MaxHops > 0 then
            getgenv().SniperConfig.CurrentHops = (getgenv().SniperConfig.CurrentHops or 0) + 1
            
            if getgenv().SniperConfig.CurrentHops > getgenv().SniperConfig.MaxHops then
                pcall(function()
                    StatusLbl.Text = "🛑 MAX HOPS! TELEPORT KE KEBUN..."
                ...
                -- Teleport ke map kebun
                task.wait(5)
                if GARDEN_PLACE_ID > 0 then
                    TeleportService:Teleport(GARDEN_PLACE_ID, LocalPlayer)
                else
                    pcall(function() StatusLbl.Text = "⚠️ ERROR: PLACE ID KEBUN BELUM DIISI!" end)
                end
                
                return -- Membatalkan proses hop dan auto-stop bot
            end
        end
        ...
        if #targetNames > 0 then
            ...
            ShuffleArray(targetNames)
            
            local TokenRAPs = ReplicatedStorage:WaitForChild("GameEvents", 3):WaitForChild("TradeEvents", 3):WaitForChild("TokenRAPs", 3)
            
            if TokenRAPs then
                local FindSellers = TokenRAPs:WaitForChild("FindSellers", 3)
                local TeleportToListing = TokenRAPs:WaitForChild("TeleportToListing", 3)
                
                if FindSellers and TeleportToListing then
                    for _, petName in ipairs(targetNames) do
                        pcall(function() StatusLbl.Text = "Mencari " .. petName .. "..." end)
                        ...
                        local success, isFound, serverId = pcall(function()
                            return FindSellers:InvokeServer("Pet", searchData)
                        end)
                        
                        if success and isFound == true and type(serverId) == "string" then
                            pcall(function() 
                                StatusLbl.Text = "🚀 SELLER DITEMUKAN! TELEPORT..."
                                StatusLbl.TextColor3 = Color3.fromRGB(0, 255, 0)
                            end)
                            TeleportToListing:InvokeServer(serverId)
                            task.wait(5)
                            return
                        end
                        task.wait(0.3)
        ...
        pcall(function() 
            StatusLbl.Text = "RANDOM HOP (SELLER KOSONG)" 
                    ...
                    if targetData ~= nil then
                        ...
                        if priceOk then
                            ...
                            if stuckCounter > 100 then
                                StatusLbl.Text = "STUCK 100X! HOPPING SERVER..."
        ...
        while true do
            if getgenv().SniperConfig.Running then
                ...
                if getgenv().SniperConfig.AutoHop then
                    ...
                    if StatusLbl.Text ~= "PURCHASING..." and StatusLbl.Text ~= "WAITING SERVER..." and not string.find(StatusLbl.Text, "STUCK") then
                       if sisa % 1 == 0 then 
                           local batasTeks = (getgenv().SniperConfig.MaxHops and getgenv().SniperConfig.MaxHops > 0) and tostring(getgenv().SniperConfig.MaxHops) or "∞"
                           local hopSkrg = getgenv().SniperConfig.CurrentHops or 0
                           StatusLbl.Text = "SCANNING... Hop: " .. sisa .. "s (" .. hopSkrg .. "/" .. batasTeks .. ")"
                           StatusLbl.TextColor3 = Color3.fromRGB(255, 50, 50)
                       end
                    end
